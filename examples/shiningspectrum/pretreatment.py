...
def uniform_layout(all_spectrum):
    """
    使用插值将不同光谱的x轴范围及精度统一。
    Interpolation is used to unify the x - axis range and precision of different spectra.
    ...
    for key in list(all_spectrum.keys()):
        ...
        x_min_list.append(min(x_data))
        x_max_list.append(max(x_data))
        x_len_list.append(len(x_data))

    x_min = min(x_min_list)
    x_max = max(x_max_list)
    x_len = max(x_len_list)
    x_new = np.linspace(x_min, x_max, x_len * 2)
    uniform_spectrum = {}
    for key in list(all_spectrum.keys()):
        f = interpolate.interp1d(all_spectrum[key][0], all_spectrum[key][1], kind='cubic')
        y_new = f(x_new)
        data_list = [x_new, y_new]
        uniform_spectrum.update({key: data_list})
